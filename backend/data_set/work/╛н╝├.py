...
economics_finance_structure = {
    ...
    # },
        "经济学": {
            ...
            "宏观经济学": {
                "重要人物": ["约翰·梅纳德·凯恩斯", "米尔顿·弗里德曼", "保罗·萨缪尔森", "约瑟夫·斯蒂格利茨"],
                "重要概念": [
                    "国民收入核算",
                    "财政政策",
                    "货币政策",
            ...
            "微观经济学": {
                "重要人物": ["阿尔弗雷德·马歇尔", "莱昂·瓦尔拉斯", "加里·贝克尔", "阿玛蒂亚·森"],
                "重要概念": [
                    "消费者行为理论",
                    "生产者理论",
                    "市场结构",
        ...
        "金融学": {
            ...
            "投资学": {
                "重要人物": ["本杰明·格雷厄姆", "彼得·林奇", "尤金·法玛", "约翰·博格"],
                "重要概念": [
                    "股票与债券",
                    "投资组合理论",
                    "行为金融学",
...
def summ_key(key):
    ...
    生动、幽默、平和、平易近人''' + '\n' + "'''{}'''，请根据以上内容介绍，总结最重要的知识笔记。输出用markdown格式展示，字数严格在200字以内。".format(content)
            result = hunyuan_api(content, title = key)
        except:
            x=1
        if result == '':
            x=1
...
data = {'guideNotes': []}
for key1 in economics_finance_structure:
    print(key1)
    level = 1
    name = key1
    ...
    sample = {
      ...
      "images": [],
      "links": {
        "to": list(economics_finance_structure[key1]),
        "from": []
      }
    }
    if content != '':
        data['guideNotes'].append(sample)
    for key2 in economics_finance_structure[key1]:
        print(key2)
        level = 2
        name = key2
        ...
        sample = {
            ...
            "images": [],
            "links": {
                "to": [key1] + flatten_list([list(economics_finance_structure[key1][key2][a])
                                                 for a in economics_finance_structure[key1][key2]]),
                "from": []
            }
        }
        if content != '':
            data['guideNotes'].append(sample)
        for temp in economics_finance_structure[key1][key2]:
            for key3 in economics_finance_structure[key1][key2][temp]:
                print(key3)
                level = 3
                name = key3
                ...
                if content != '':
                    data['guideNotes'].append(sample)
    if len(data['guideNotes']) > 20:
        break
import json
with open('经济_result20.json', 'w', encoding='utf-8') as f:
